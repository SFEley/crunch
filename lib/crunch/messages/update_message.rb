module Crunch
  ...
  class UpdateMessage < Message
    ...
    attr_reader :collection_name, :update, :selector, :upsert, :multi
    ...
    # @option opts [Hash, Fieldset] :update ({}) The values we're updating -- either the complete document or a hash of atomic update operators (i.e. '$set' and friends)
    # @option opts [Hash, Fieldset] :selector ({}) If specified, describes the document(s) to be updated
    ...
    def initialize(collection, opts={})
      ...
      @update = Fieldset.new opts[:update]
      if opts[:id]
        @selector = Fieldset.new (opts[:selector] || {}).merge('_id' => opts[:id])
      else
        @selector = Fieldset.new opts[:selector]
      end
      @upsert = opts[:upsert]
      @multi = opts[:multi]
    end
